...
const EditPost2 = () => {
    ...
    const fetchPost = async () => {
        try {
            const response = await axios.get(`http://localhost:8090/api/board3/view/${id}`);
    ...
    const handleEditPost = async () => {
        try {
            ...
            await axios.put(`http://localhost:8090/api/board3/modify/${id}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });

            navigate('/Mun'); // 글 수정 후 Gong 페이지로 이동
        } catch (error) {
